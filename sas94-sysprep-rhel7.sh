...
echo;echo
echo "Setting up Password-less sudo for users in OS wheel group - checking/editing /etc/sudoers"


echo "###########################"
echo "Original /etc/sudoers Content pre-updates"
echo 
    sudo cat /etc/sudoers | grep -v "#" | awk NF

echo;echo
echo "###########################"

echo "Checking /etc/sudoers for existing settings"
echo 

if [[ $(cat /etc/sudoers | grep -v -e "#" -e "Defaults"  | grep wheel |  awk NF | awk '{print $3 $4}') = *NOPASSWD:*ALL* ]]
    then
    echo "Password-less sudo for wheel group users already enabled. Nothing to do. Exiting without changes."
    else 
        echo "Setting up sudo for members of wheel without a password"
        echo "Creating Backup of /etc/sudoers to  /etc/sudoers.bkup"
        cp /etc/sudoers /etc/sudoers.bkup
        echo '%wheel  ALL=(ALL)       NOPASSWD: ALL' | sudo EDITOR='tee -a' visudo
        echo "Updated /etc/sudoers contents: "
        cat /etc/sudoers | grep -v "#" | awk NF | grep -v "Defaults"
